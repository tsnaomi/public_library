...
def library_catalog(library):
	if library.inventory == {}:
		return "Oh my! %s has no books!" % (public_library.name)
	else:
...
print "Shelf %s contains the following books: %s" % (S3.label, str(S3.books)) # >>> Shelf KLMNO contains the following books: ['Bartleby the Scrivener']
print "Shelf %s contains the following books: %s" % (S4.label, str(S4.books)) # >>> Shelf PQRST contains the following books: ['Harry Potter1', 'Harry Potter2']
print "Altogther, %s contains the following books: %s" % (public_library.name, str(library_catalog(public_library))) # >>> Altogther, The Public Library contains the following books: [['Harry Potter1', 'Harry Potter2'], ['Bartleby the Scrivener']]

print unshelf(Harry_Potter1, S4, public_library) # >>> Harry Potter1 has been removed from Shelf PQRST

print "Shelf %s contains the following books: %s" % (S4.label, str(S4.books)) # >>> Shelf PQRST contains the following books: ['Harry Potter2']
print "Now, %s contains the following books: %s" % (public_library.name, str(library_catalog(public_library))) # >>> Now, The Public Library contains the following books: [['Harry Potter2'], ['Bartleby the Scrivener']]
